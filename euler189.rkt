      ...
      [this-colour-count
        (if invalid-colour? 0
            (recurse (sub1 num-triangles)
                     ...
                     colour-index new-colour-queue
                     ((if even-column? / *) mult num-colours)))]
      [next-colour-count
        (recurse num-triangles
                 row-index column-index (sub1 colour-index)
                 left-colour-index colour-queue mult)])
...
(triangle-up   (+ CENTER (*  0 SIZE)) (* 0 HEIGHT) "RED")

(triangle-up   (+ CENTER (* -1 SIZE)) (* 1 HEIGHT) "BLUE")
(triangle-down (+ CENTER (*  0 SIZE)) (* 2 HEIGHT) "GREEN")
(triangle-up   (+ CENTER (*  1 SIZE)) (* 1 HEIGHT) "BLUE")
